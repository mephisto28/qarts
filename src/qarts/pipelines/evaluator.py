...
class EvaluatorProcessor(Processor):
    ...
    def __init__(
        ...
        if output_dir is not None:
            self.output_dir = os.path.join(output_dir, self.model_name, f'eval_epoch{self.epoch:02d}')
        else:
            d = os.path.dirname
            project_dir = d(d(d(d(os.path.abspath(__file__)))))
            self.output_dir =  os.path.join(project_dir, 'experiments/output', self.model_name, f'eval_epoch{self.epoch:02d}')
        os.makedirs(self.output_dir, exist_ok=True)

    @property
    def input_fields(self) -> list[str]:
        return [f'inference_{self.model_name}', f'factors_{self.targets_name}']

    @property
    def output_fields(self) -> list[str]:
        return [f'eval_{self.model_name}']
    ...
    def process(self, context: GlobalContext):
        ...
        preds = preds_block.get_copy(self.pred_fields)
        targets = targets_block.get_copy(self.target_fields)
        pred_s = preds.data.transpose(1, 2, 0)[:, ::self.sample_stride]
        ...
        logger.info(
            f'{self.metrics[-1]["date"]} {self.model_name} metrics'
    ...
    def finalize(self) -> T.Dict[str, T.Any]:
        ...
        df_ic = pd.DataFrame(ic, index=dates, columns=self.pred_fields)
        df_rankic = pd.DataFrame(ric, index=dates, columns=self.pred_fields)
        df_rankic_long = pd.DataFrame(ric_long, index=dates, columns=self.pred_fields)
        df_hit = pd.DataFrame(hit, index=dates, columns=self.pred_fields)
        df_auc = pd.DataFrame(auc, index=dates, columns=self.pred_fields)
        ...
        # summary table
        summary_rows = []
        for j, f in enumerate(self.pred_fields):
