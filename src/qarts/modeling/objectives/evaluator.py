...
class HybridEvaluator(nn.Module):
    ...
    def set_input_specs(self, input_columns: list[str], target_columns: list[str]):
        ...
        if getattr(self, 'pred_indices', None) is None:
            ...
            for schema in self.schemas:
                self.pred_indices[schema.name] = schema.pred_indices
                self.target_indices[schema.name] = [target_columns.index(f) for f in schema.target_fields]
    ...
    def forward(self, preds, targets):
        ...
        for schema in self.schemas:
            ...
            pred_idx = self.pred_indices[name]
            target_idx = self.target_indices[name]
            p_sub = preds[..., pred_idx]
            t_sub = targets[..., target_idx]
            ...
            for metric_name, metric_fn in metric_fns.items():
                ...
                if isinstance(val, dict):
                    ...
                    for i, f in enumerate(schema.target_fields):
